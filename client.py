...
class Receive(threading.Thread):
    ...
    def run(self):
        ...
        """
        while True:
            message = self.sock.recv(2048).decode('ascii')
            if message:
                if self.messages:
...
class Client:
    ...
    def start(self):
        ...
        # Input UserName
        self.name = input(response.decode())	
        self.sock.send(str.encode(self.name))
        response = self.sock.recv(2048)
        # Input Password
        password = input(response.decode())	
        self.sock.send(str.encode(password))
...
def main(host, port):
    ...
    text_input.insert(0, "Type your message here.")
